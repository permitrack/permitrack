    ...
    this.ac
            = new Array("seedDateString",
                        ...
                        new Function("varName",
                                     "this.datePatternStrict='MM/dd/yyyy';  return this[varName];"));
    this.ac
            = new Array("completedDate",
                        "Completed Date is not a valid date. (mm/dd/yyyy)",
                        new Function("varName",
                                     "this.datePatternStrict='MM/dd/yyyy';  return this[varName];"));
}
...
function validateForm(form)
{
    ...
    /*
     ...
     == true)
     {
     if (confirm("Are you sure you want to permanently delete this project?  Click 'OK' to continue deleting this project."))
     {
     return true;
     }
     else
    ...
    if (document.getElementById("projectTypeId").value
            ...
            == '')
    {
        $('#dialog').html("Invalid Project Type selected!").dialog('open');
        return false;
    }
            ...
            < projectBmpArray.length; bmpIndex++)
    {
                ...
                == 'false')
        {
            ...
            if (projectBmpArray[bmpIndex].bmpDescription.length
                    > 500)
            {
                $('#dialog').html("Project Inspection BMP Description must be less than 500 characters.").dialog('open');
                return false;
            }
        }
    }
            ...
            - 1; x++)
    {
                ...
                != null)
        {
                    ...
                    != "")
            {
                ...
                // Ensure added contacts organization fields are not blank
                if (!(document.getElementById("contacts.contact"
                                                      + x
...
{
    var index = 0;
    var maxIndex = projectDocumentArray.length
            + 4;
    for (index
                 = projectDocumentArray.length; index
            < maxIndex; index++)
    {
...
function getDeletedDocuments()
{
            ...
            < projectDocumentArray.length; index++)
    {
        ...
        {
            inputText
                    += "<input type='hidden' id='documents.document"
                               + index
                               + ".isDeletedText' name='documents.document"
                               + index
                               ...
                               + index
                               + ".id' value='"
                               + projectDocumentArray[index].documentId
                    + "'/>";
        }
    }
    return inputText;
...
function displayDocumentList()
{
    ...
    document.getElementById('projectDocumentList').innerHTML
            = inputHTML;
    //$.uniform.update(document.getElementById('projectDocumentList'));
    $(function ()
      {
          //          $("#projectDocumentList input").uniform();
      });
}
function createDocumentInput(id, projectDocument)
{
    var inputString = "<tr><input type='hidden' id='documents.document"
                              + id
                              + ".id' name='documents.document"
                              + id
                              + ".id' value='"
            ...
            == '')
            ? ""
            : "<a class='btn btn-mini btn-danger' href='javascript:deleteDocumentInput(\""
                      + id
            + "\");'>Delete</a>")
            + "</td></tr>";
    return inputString;
...
{
    if (projectContactArray[index].projectContactId
                != null
...
{
    //  document.getElementById('contacts.contact' + id + '.organizationName').value = '';
    document.getElementById('contacts.contact'
                                    + id
                                    + '.isDeletedText').value
            = 'true';
    ...
    document.getElementById('contacts.contact'
                                    ...
                                    + '.city').value
            = '';
    //document.getElementById('contacts.contact' + id + '.stateCode').value = '';
    document.getElementById('contacts.contact'
                                    + id
                                    + '.zip').value
            = '';
...
function initInternalProjectContact(id, clientId, contactTypeId)
{
    ...
    /*
     ...
     id);
     */
    // Create state textbox with a value if necessary.
    if (stateValue
            == null)
    {
        stateValue
                = '';  // Stops the word "undefined" from appearing in the readonly field for a Client contact with no info.
    }
    var inputString = "<input  class='span1' type='text' id='contacts.contact"
                              + id
                              + ".stateCode' name='contacts.contact"
...
function internalProjectContactOnChange(id)
{
    ...
    if (contactId
            == '-1')
    { // Dropdown is set to Add New
        document.getElementById('contacts.contact'
                                        + id
                                        + '.contactId').value
                = '-1';
        // Enable fields.  If 'Add New' is selected then fields are editable
        document.getElementById('contacts.contact'
                                        + id
                                        + '.firstName').readOnly
                = false;
...
function createBmpInput(id, bmp)
{
    var inputString = "<tr><td ><input type='hidden' id='bmps.bmp"
            ...
            : "")
                              ...
                              + ".isRequiredText' value='"
                              + bmp.isRequired
                              + "' onchange='return isRequiredOnChange(\""
                              + id
                              + "\");'/></td>"
                              + "<td>"
                              + "<input type='hidden' id='bmps.bmp"
                              ...
                              + id
                              + ".bmpDescription' cols='40' rows='3' onchange='return bmpDescriptionOnChange(\""
                              + id
                              + "\");'/>"
                              + bmp.bmpDescription
                              + "</textarea></td>"
                              + "<td><a class='btn btn-mini btn-danger' href='javascript:deleteBmpInput(\""
                              + id
            + "\");'>Delete</a></td></tr>";
    return inputString;
}
function createNewBmpInput(id, bmp)
{
    var inputString = "<tr><td ><input type='hidden' id='bmps.bmp"
            ...
            : "")
                              ...
                              + id
                              + ".bmpCategoryId' onchange='return categoryOnChange(\""
                              + id
                              + "\");'>"
                              ...
                              + ".bmpId' name='bmps.bmp"
                              + id
                              + ".bmpId' onchange='return bmpOnChange(\""
                              + id
                              + "\");'>"
...
{
    var status = document.getElementById('projectStatusCode').value;
    if (status
            == 8)
    {
        populateCompletedDate();
    }
    else
    {
        removeCompletedDate();
...
function removeCompletedDate()
{
    document.getElementById('completedDate').value
            = '';
    document.getElementById('completedDate').disabled
            = true;
}
function populateCompletedDate()
{
