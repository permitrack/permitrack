...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[sp_permit_assets_by_client]') 
and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_permit_assets_by_client]
GO


CREATE PROCEDURE dbo.sp_permit_assets_by_client
	-- Required Parameters
	@clientId int = NULL
AS

-- all assets for a client that are attached to the client's permits
SELECT 
...
WHERE permit.client_id = @clientId --client_id
