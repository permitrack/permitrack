...
WHILE (SELECT COUNT(*) FROM @tanks) > 0
  BEGIN
	...
	-- WORKING EMISSIONS --
	SELECT @workingEm = CORE.BL_EM_FACTOR, @workingThru = (SUM(CORE.METER_READING) / 1000) 
	FROM (
	...
	-- LOADING EMISSIONS --
	SELECT @loadingEm = CORE.BL_EM_FACTOR, @loadingThru = (SUM(CORE.METER_READING) / 1000) 
	FROM (
		SELECT SCC.SCC_NUMBER, ss.BL_EM_FACTOR, usage.METER_READING
		FROM ENV_SCC_INFO AS SCC
		INNER JOIN ENV_SUBSTANCE_SCC_INFO AS subSCC 
