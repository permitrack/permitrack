--exec sp_other_haps @yearInt=2009, @facilityId=1

if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[sp_other_haps]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_other_haps]
GO

CREATE PROCEDURE dbo.sp_other_haps
	-- Required Parameters
	@yearInt int = NULL, 
	@facilityId int = NULL
AS
