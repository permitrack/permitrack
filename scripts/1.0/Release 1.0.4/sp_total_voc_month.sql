...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[sp_total_voc_month]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_total_voc_month]
GO


CREATE PROCEDURE dbo.sp_total_voc_month
	-- Required Parameters
...
INNER JOIN ENV_ASSET_TYPE as assetType ON assetType.ASSET_ID = asset.id
	WHERE asset.client_id = @clientId
