...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[sp_total_haps_by_asset]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_total_haps_by_asset]
GO


CREATE PROCEDURE dbo.sp_total_haps_by_asset
	-- Required Parameters
...
SELECT
	...
	FROM (
		...
		) AS TOTALS GROUP BY TOTALS.A_ID, TOTALS.READING, TOTALS.LBS_HAPS, TOTALS.UM
