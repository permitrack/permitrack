...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[sp_total_solids_by_asset]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_total_solids_by_asset]
GO


CREATE PROCEDURE dbo.sp_total_solids_by_asset
	-- Required Parameters
...
SELECT
	...
	FROM (
		...
		FROM (
			...
			GROUP BY asset.id, usage.METER_READING, usage.UNIT_OF_MEASURE_CD, source.PCT_SOLIDS_VOLUME 
		) AS TOTALS GROUP BY TOTALS.A_ID, TOTALS.READING, TOTALS.UM, TOTALS.PCT_SOLIDS_VOLUME
