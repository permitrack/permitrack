...
--EXEC sp_form_12_by_asset @yearInt=2009, @facilityId=1, @clientId=682

if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[sp_form_12_by_asset]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_form_12_by_asset]
GO

CREATE PROCEDURE dbo.sp_form_12_by_asset
	-- Required Parameters
	@yearInt int = NULL,
	@facilityId int = NULL,
	@clientId int = NULL
...
EXEC sp_greatest_hap @yearInt=@yearInt, @facilityId=@facilityId

SELECT TOP 1 @greatestHapId = SUBST_ID FROM #GREATEST_HAP
...
INSERT INTO @assets
SELECT ASSET_ID, ASSET_NUMBER FROM #TEMP_ASSET_TOTALS
...
WHILE (SELECT COUNT(*) FROM @assets) > 0
  BEGIN
	...
	WHILE (SELECT COUNT(*) FROM @sources) > 0
	  BEGIN
		...
		BEGIN
			...
			-- SET @numerator = @vocContent * @density * @sourceTotal -- pam's old calc, removed density 2/22/2011
			SET @numerator = @vocContent * @sourceTotal
	...
	BEGIN
		...
		IF ( @emissionFactor is null ) -- asset was marked as "paint" but no paint sources attached
		BEGIN
			SET @actualEmissions = 0
		END

		IF ( @emissionFactor is not null )
		BEGIN
...
INSERT INTO #TANKS
EXEC sp_form_12_tanks @yearInt=@yearInt, @clientId=@clientId, @facilityId=@facilityId

SELECT TOP 1 @tanktotal = TANK_TOTAL FROM #TANKS

INSERT INTO @results
(ASSET_ID, ASSET_NUMBER, CO, NH3, NOX, LEADS, PM10, PM2_5, SOX, VOC, GREATEST_HAP, OTHER_HAPS)
VALUES
(0, 'Form 10.1', 0, 0, 0, 0, 0, 0, 0, @tanktotal, 0, 0 )
