...
import com.sehinc.portal.action.auth.LoginAction;
    ...
    extends javax.servlet.http.HttpServlet
{
        ...
        throws ServletException, IOException
    {
            ...
            != null)
        {
            ActionForward
                forward =
                LoginAction.loginUser(req.getParameter("openid.ext1.value.email"),
                                      null,
                                      req,
                                      resp,
                                      null,
            ...
            {
                System.out
                    .println("login with openid succeeded for "
                             + identifier.getIdentifier());
                resp.sendRedirect("/sehsvc"
                                  + forward.getPath());
                return;
            }
            else
            {
                ...
                errors.add(ActionErrors.GLOBAL_MESSAGE,
                           new ActionMessage(
                           "Gmail email address not found.", false));
                SessionService.setSessionAttribute(Globals.ERROR_KEY,
                                     errors, req);
        ...
        else
        {
            ...
            errors.add(ActionErrors.GLOBAL_MESSAGE,
                       new ActionMessage(
                       "Apologies, but Google authorization failed.", false));
            SessionService.setSessionAttribute(Globals.ERROR_KEY,
                                               errors, req);

        }
        System.out
            .println("login with openid failed");
        resp.sendRedirect("/sehsvc/login");
    }
        ...
        throws IOException
    {
        try
        {
            ...
            List
                discoveries =
                manager.discover(userSuppliedString);
            ...
            httpReq.getSession()
                .setAttribute("openid-disc",
                              discovered);
            // obtain a AuthRequest message to be sent to the OpenID provider
            AuthRequest
                authReq =
                manager.authenticate(discovered,
                                     returnToUrl);
            FetchRequest
                fetch =
    ...
    public Identifier verifyResponse(HttpServletRequest httpReq)
    {
        try
        {
            ...
            VerificationResult
                verification =
                manager.verify(receivingURL.toString(),
                               response,
                               discovered);
                ...
                != null)
            {
                ...
                if (authSuccess.hasExtension(AxMessage.OPENID_NS_AX))
                {
                    ...
                    String
                        email =
                        (String) emails.get(0);
                    log.info("OpenIdlogin done with email: "
                             + email);
                }
